...
sub jack_port_to_nama {
	my $jack_port = shift;
	grep{ /$config->{ecasound_jackclient_name}/ and $jack->{is_own_port}->{$_} } @{ $jack->{connections}->{$jack_port} };
}
...
sub make_connections {
	...
	map{  
		my $track = $_; 
 		my $name = $track->name;
 		my $ecasound_port = "$config->{ecasound_jackclient_name}:$name\_$direction\_";
...
sub register_own_ports { # distinct from other Nama instances 
	...
	{ 
		...
		grep{ /^$config->{ecasound_jackclient_name}/ } 
		qx(jack_lsp)
	} 
}
