...
sub _playat_output {
	my $track = shift;
	return unless $track->adjusted_playat_time or
		my $offset = $setup->{sibling_latency}->{$track->name}
						- $setup->{track_latency}->{$track->name};
	join ',',"playat" , $track->adjusted_playat_time + $offset
}
sub _select_output {
	my $track = shift;
