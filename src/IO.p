...
sub _playat_output {
	my $track = shift;
	return unless $track->adjusted_timeline_position;
		# or $track->latency_offset;
	join ',',"playat" , $track->adjusted_timeline_position
		# + $track->latency_offset
}
sub _select_output {
	my $track = shift;
	no warnings 'uninitialized';
	my $start = $track->adjusted_startpoint + $config->hardware_latency();
	my $end   = $track->adjusted_endpoint;
